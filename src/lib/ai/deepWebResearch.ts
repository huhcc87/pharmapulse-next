...
export class DeepWebResearch {
  ...
  static async researchProduct(params: {
    ...
    if (barcode && barcode.length >= 8) {
      ...
      if (gs1Result) {
        if (gs1Result.manufacturer) {
          ...
          confidenceScore += 30; // GS1 manufacturer confirmation
        }
    ...
    if (barcode) {
      ...
      if (barcodeDbResult) {
        // Product name - take if not already set, or if higher confidence
        if (barcodeDbResult.productName && !suggestions.productName) {
          ...
          confidenceScore += 20;
        } else if (barcodeDbResult.productName && suggestions.productName === barcodeDbResult.productName) {
          // Agreement - boost confidence
          const existing = fieldConfidence.find(f => f.field === 'productName');
          if (existing) {
            existing.confidence = Math.min(existing.confidence + 15, 90);
            confidenceScore += 10; // Agreement bonus
          }
        }

        // Manufacturer - check agreement
        if (barcodeDbResult.manufacturer) {
          ...
          } else if (suggestions.manufacturer.toLowerCase() === barcodeDbResult.manufacturer.toLowerCase()) {
            // Agreement - boost manufacturer confidence
            const existing = fieldConfidence.find(f => f.field === 'manufacturer');
            if (existing) {
              existing.confidence = Math.min(existing.confidence + 10, 90);
              confidenceScore += 15; // Manufacturer agreement
            }
          }
        }
        ...
        // Description (neutral, non-marketing)
    ...
    if (productName || suggestions.productName) {
      ...
      if (searchQuery && !searchQuery.includes('Product') && !searchQuery.includes('Generic')) {
        ...
        if (marketResult) {
          // MRP - only if consistent across sources
          if (marketResult.mrp) {
            if (!suggestions.mrp) {
            ...
            } else if (Math.abs(suggestions.mrp - marketResult.mrp) < 10) {
              // MRP agreement (within ₹10)
              const existing = fieldConfidence.find(f => f.field === 'mrp');
              if (existing) {
                existing.confidence = Math.min(existing.confidence + 10, 85);
                confidenceScore += 10; // MRP agreement
              }
            } else {
      ...
      if (searchQuery && !searchQuery.includes('Product')) {
        ...
        if (regulatoryResult) {
          ...
          if (regulatoryResult.composition && !suggestions.composition) {
            ...
            confidenceScore += 15; // Composition found
          }

          // Schedule (high-risk - only if multiple sources agree)
    ...
    // HSN Code inference (if not provided and category known)
    ...
    // Calculate unit price if MRP and pack size known
    if (suggestions.mrp && suggestions.packSize) {
      const packUnits = this.extractPackUnits(suggestions.packSize);
      if (packUnits > 0) {
        suggestions.unitPrice = Math.round((suggestions.mrp / packUnits) * 0.9 * 100) / 100; // 10% discount estimate
    ...
    // Cap overall confidence at 95% (never 100% from web alone)
  ...
  /**
   * Check GS1/GTIN database for manufacturer/brand owner
   * Mock implementation - in production, integrate with GS1 lookup API
   */
  private static async checkGS1Database(barcode: string): Promise<Partial<ResearchResult> | null> {
    // Simulate GS1 lookup delay
    await new Promise(resolve => setTimeout(resolve, 800));

    // Mock: For known barcodes, return manufacturer
    // In production: Call GS1 lookup API or use GTIN database
    if (barcode.startsWith('890') && barcode.length === 13) {
      // This is where you'd integrate with actual GS1 API
      // For now, return null to simulate no GS1 match
      return null;
    }
  ...
  /**
   * Query barcode product databases (EAN-DB, BarcodeLookup)
   * Mock implementation - in production, integrate with actual APIs
   * 
   * IMPORTANT: Never return generic placeholders like "Pharmaceutical Product XXXX"
   * Return null if actual product name cannot be determined
   */
  private static async queryBarcodeDatabases(barcode: string): Promise<Partial<ResearchResult> | null> {
    await new Promise(resolve => setTimeout(resolve, 1000));

    // Mock: Enhanced extraction - no generic placeholders
    // In production: Call EAN-DB API, BarcodeLookup API, etc.
    // Parse actual API responses and extract structured data
    
    // For 13-digit Indian pharmaceutical barcodes
    if (barcode.startsWith('890') && barcode.length === 13) {
      // In production: Query actual barcode databases
      // For now, return null to indicate no reliable data
      // Better to return null than generic placeholder
      
      // NOTE: If barcode database returns actual product name, return it here
      // Only return productName if it's a real brand name, not "Product XXXX"
      return null; // No reliable data from barcode databases alone
    }
  ...
  /**
   * Query India pharmacy/market listings for MRP
   * Mock implementation - in production, query retail pharmacy websites
   * 
   * IMPORTANT: Extract only numeric MRP values, never copy entire descriptions
   * Treat MRP as pack-level and location-dependent - show warnings if uncertain
   */
  private static async queryMarketListings(productName: string): Promise<Partial<ResearchResult> | null> {
    await new Promise(resolve => setTimeout(resolve, 1200));

    // Mock: Extract MRP patterns from market listings
    // In production: Query pharmacy websites (1mg, PharmEasy, Netmeds, etc.)
    // Parse HTML/JSON responses and extract numeric MRP values only
    // Never copy full descriptions - only extract numeric pricing
    
    // For demonstration: Simulate MRP extraction for known products
    // In production, this would parse actual market listing responses
    const nameLower = productName.toLowerCase();
    
    // Match product patterns and extract MRP (if found in market listings)
    if (nameLower.includes('paracetamol') || nameLower.includes('dolo')) {
      return { mrp: 35.00, packSize: '15 tablets' }; // Market-estimated
    }
    if (nameLower.includes('metformin') || nameLower.includes('glycomet')) {
      return { mrp: 50.00, packSize: '10 tablets' }; // Market-estimated
    }
    if (nameLower.includes('rabeprazole') || nameLower.includes('raboserv')) {
      return { mrp: 80.00, packSize: '10 tablets' }; // Market-estimated - matches Raboserv-LP
    }

    // If no match found, return null (better than guessing)
    return null;
  }

  /**
   * Query regulatory/public sources (CDSCO)
   * Mock implementation - in production, query CDSCO database
   */
  private static async queryRegulatorySources(productName: string): Promise<Partial<ResearchResult> | null> {
    await new Promise(resolve => setTimeout(resolve, 1000));

    // Mock: Extract composition and schedule from regulatory data
    // In production: Query CDSCO API or parse regulatory pages
    
    const nameLower = productName.toLowerCase();
    
    // Simulate regulatory data extraction
  ...
  /**
   * Generate neutral description from extracted facts
   * Removes marketing language, focuses on composition and category
   */
  ...
  /**
   * Infer HSN code from category
   * Returns suggested HSN based on category classification
   */
  private static inferHsnFromCategory(category: string): string | null {
    ...
    if (categoryLower.includes('analgesic') || categoryLower.includes('medicine') || categoryLower === 'general') {
      return '30049099'; // Standard medicine HSN
    }
  ...
  /**
   * Extract pack units from pack size string
   * e.g., "15 tablets" -> 15
   */
